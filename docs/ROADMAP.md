...
- [x] **§7.9 の文字列層（テキスト文字列の復号 + 日付・2026-08-27）— 要求は消費者から立った。** 4 つの消費者が同じものを 4 通りに実装していた（reader は自前 + pdf-lib からの借用 2 本・verify は `decodeText()` × 19・pdf-constraints は × 6 + 自前 `parsePdfDate`・writer は書き側を自前）。`src/text/` に 3 本: `pdfdoc-encoding.ts`（Annex D.3 Table D.3 を条文から起こした 256 バイトの表・定義済み 232 / 未定義 24）・`text-string.ts`（`decodeTextString` / `encodeTextString` / `stripLanguageEscape`）・`date.ts`（`parsePdfDate` / `formatPdfDate` / `PdfDate`）
  - **条文で決まったこと 3 つ。**（1）言語エスケープ列は「Unicode テキスト文字列の**どこにでも**現れうる」（§7.9.2.2.2）ので先頭 1 個ではなく全体から取り除く。（2）**PDFDocEncoded 文字列には適用しない** — PDFDocEncoding に ESCAPE は無く、バイト 0x1B は U+02D9 DOT ABOVE（Table D.3）なので、そこで剥がすと本文を消す（reader の現行実装はここで剥がしている）。（3）日付を**書く**形は末尾のアポストロフィ無し（ISO 32000-2 の形。NOTE 2 が言うのは PDF 1.7 の形を*読むとき*に受け入れよ、である）。読み側は両方受け入れる
  - 🔴 **Table D.3 の自己矛盾 3 件を実測**（`TABLE_D3_DEFECTS` に記録）: 0x16 が Unicode 列 `U+0017` / 名前 `(SYNCRONOUS IDLE)`（= U+0016）で 0x17 と同じ値を指す・0x04 の名前が 0x03 と同じ・0x38 の名前が `DIGIT EIGJT`。**3 件とも Notes 列が `U`（未定義）の行**なので適合文書は踏まない。**pdf-lib も 0x16 → U+0017 という同じ欠陥を持つ**（同じ表から起こしたため）。また §7.9.2.2.1 NOTE 4 の字形名（dieresis…）は Table D.3 のコード点と合わない（先頭は U+00A8 = 0xA8 であってバイト 0xEF ではない）
  - **T-3 を 7 通り実測 = すべて赤になる**: UTF-16BE の BOM 検査を外す（10 件）・UTF-8 の BOM 検査を外す（4 件）・言語エスケープの除去を外す（7 件）・補助文字を 1 ユニットに落とす（2 件）・BOM と衝突する PDFDocEncoding の回避を外す（1 件）・日付のアポストロフィを任意にする（1 件）・未定義コード点も書けるようにする（1 件）
  - **差分オラクル（`scripts/text-oracle.mjs`・pdf-lib は ad hoc に入れる）**: コーパス 2,917 検体中 2,916 読了。**テキスト文字列 2,612 件中 2,606 件がバイト一致・差 6 件はすべて UTF-8 BOM**（R-7.9.2.2.1-4。pdf-lib 1.x は扱わないので、条文が正・改善として帰属）。**日付 941 件中 938 件が両実装で読めて値も一致・3 件は両方 null**（空振りではない）。読めない 3 件のうち 2 件は UTF-8 BOM 付きの日付で、`decodeTextString` を通せば読める（**938 → 940**）。残り 1 件は ISO 8601 で書かれた日付で、§7.9.4 の形ではない
  - **往復（面 3）**: PDFDocEncoding の範囲内・範囲外（日本語）・補助文字・言語エスケープ入りの 4 軸 + 定義済み 232 バイト全数。言語エスケープだけは恒等でなく、復号が取り除いた形に戻る（条文どおり）
  - **受入の面 4（消費者が置き換えられた）は未了** — 第 1 消費者 `@shuji-bonji/pdf-constraints` への置き換えはトラック B の B1 で行う
  - **第 2 消費者 = pdf-verify-mcp を 2026-08-29 に置き換えた（0.19.0）。** `src` の pdf-lib import 5 → 0・`npm ls --omit=dev pdf-lib` = `(empty)`。受入 3 面（撤去 / 出力の A/B を全件帰属 / 独立オラクル）で採り、**誤った `pass` は 0 件**。B2 で置いた 3 つ（`xref-walk.ts` = チェーンの歩きと回復方針 / `document.ts` = `openDocument` と `DocumentScope` / `cos.ts` = COS の読み口）は **reader の撤去でもそのまま型になる** —— コピーするか共有に上げるかは着手前に決める（受け皿の候補は ADR-0009 の `@normativepdf/document`）。続けて 0.20.0 / 0.21.0 / 0.21.1 で**判定の射程（`scope`）を 7 ツールすべての出力に載せた**（`reconstructed` = 相互参照表が verify の推測であること等）。記録は `mcp/pdf-verify-mcp/docs/handoff/pdflib-removal.md` と `scope-in-output.md`
- [ ] reader から使わせる（**第 3 消費者**・着手前の 1 枚 = [`mcp/pdf-reader-mcp/docs/handoff/pdflib-removal.md`](../../../mcp/pdf-reader-mcp/docs/handoff/pdflib-removal.md)・2026-08-29 実測）— reader 0.13.0 は **src 10 ファイル・約 5,700 行**が pdf-lib に触れている（入口は `pdflib-service.ts` 562 行で、8 ファイルがそこを import）。import しているのは COS 8 種だけで高レベル API はほぼ使っていない = verify と同じ形。🔴 **reader にはゴールデンの計器が無い**ので、最初の仕事は撤去ではなく撤去前の出力を凍結する計器を書くこと（19 ツール × 2,950 検体 ≒ 55,000 呼び出し）。🔴 **pdfjs-dist は撤去対象ではない** —— テキスト抽出とページ描画は別の問いで、normativepdf にその層はまだ無い（下記「需要駆動候補」）。6 ファイルが両方に触れているので切り分けを先に測る。**版数報告だけの部分移行は取り下げた（2026-08-13）**。reader の版数経路は pdf-lib 依存であり、1 フィールドのために normativepdf でもう一度フルパースして strict throw を pdf-lib で受けるフォールバック二重構造になる。版数は面が小さく API 適合性の計器としても割に合わない（[[saturated-faces-cannot-carry-a-difference]]）。**Phase 3 で移行したのは writer であって reader ではない**（訂正 2026-08-18）。reader はまだ pdf-lib の上にある。移行の要求は第 1 消費者 = writer から立ったので、reader は次の消費者として別に起こす —— 「一括で移行する」と書いていたが、そうはならなかった
