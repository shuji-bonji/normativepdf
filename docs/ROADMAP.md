...
- [ ] 条文調査: ISO 32000-2 §7.3（オブジェクト）§7.5（ファイル構造）を pdf-spec-mcp で引き、COS 型ユニオンに写す
- [ ] プロジェクト初期化（tsconfig = ADR-0002 §1 転記・biome・vitest・ESM）
- [ ] ByteCursor（境界検査を 1 箇所に閉じる）+ レキサ
- [ ] オブジェクトパーサ（8 種 + 間接参照）
...
- [ ] 単体テスト（vitest・T-1〜T-4 の規律・「修正を戻すと落ちる」実測）
