...
**`allocate` の採番は既存の規律を踏襲する。** 段階 1 で「定義済みの最大番号 + 1」にしたら、
`/Info 21 0 R` が指す先の無い検体で**宙ぶらりんの参照が解決されてしまった**。
`highestReferencedNumber`（`file-writer.ts`）が既にこの計算を持っているので、それを使う。
